...
public class EntityMechanicalZombie extends EntityMob {
	...
	protected void initEntityAI() {
		...
		this.tasks.addTask(2, new EntityAIMechanicalZombieAttack(this, 1.0D, false));
	...
	public static void registerFixesZombie(DataFixer fixer) {
		EntityLiving.registerFixesMob(fixer, EntityMechanicalZombie.class);
	}
	...
	public void onKillEntity(EntityLivingBase entityLivingIn) {
		...
		if ((this.world.getDifficulty() == EnumDifficulty.NORMAL || this.world.getDifficulty() == EnumDifficulty.HARD)
			...
			EntityZombieVillager EntityMechanicalZombievillager = new EntityZombieVillager(this.world);
			EntityMechanicalZombievillager.copyLocationAndAnglesFrom(entityvillager);
			this.world.removeEntity(entityvillager);
			EntityMechanicalZombievillager.onInitialSpawn(
					this.world.getDifficultyForLocation(new BlockPos(EntityMechanicalZombievillager)),
					new EntityMechanicalZombie.GroupData(false));
			EntityMechanicalZombievillager.setProfession(entityvillager.getProfession());
			EntityMechanicalZombievillager.setChild(entityvillager.isChild());
			EntityMechanicalZombievillager.setNoAI(entityvillager.isAIDisabled());

			if (entityvillager.hasCustomName()) {
				EntityMechanicalZombievillager.setCustomNameTag(entityvillager.getCustomNameTag());
				EntityMechanicalZombievillager.setAlwaysRenderNameTag(entityvillager.getAlwaysRenderNameTag());
			}

			this.world.spawnEntity(EntityMechanicalZombievillager);
	...
	public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
		...
		if (livingdata == null) {
			livingdata = new EntityMechanicalZombie.GroupData(
					this.world.rand.nextFloat() < net.minecraftforge.common.ForgeModContainer.zombieBabyChance);
		}

		if (livingdata instanceof EntityMechanicalZombie.GroupData) {
			EntityMechanicalZombie.GroupData EntityMechanicalZombie$groupdata = (EntityMechanicalZombie.GroupData) livingdata;

			if (EntityMechanicalZombie$groupdata.isChild) {
				this.setChild(true);
	...
	// TODO
	// -----------------------------------------------------------------------------------------------------
	// TODO THIS IS WHERE THE LIST ? / ARRAY will be called for dropping types
	// of items in random chance
	@Override
