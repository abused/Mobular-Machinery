    ...
    public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        ...
        if(!world.isRemote) {
            ...
            if (player.isSneaking()) {
                if (tag != null) {
                    if (state.getBlock() == ModResources.blockEnergyRelay) {
                ...
                if (tag != null && tag.hasKey("x") && tag.hasKey("y") && tag.hasKey("z")) {
                    TileEntity tileCheck = world.getTileEntity(new BlockPos(tag.getInteger("x"), tag.getInteger("y"), tag.getInteger("z")));
                    if (tileCheck != null && tileCheck instanceof TileEntityEnergyRelay) {
                        if (state.getBlock() == ModResources.blockEnergyRelay) {
                            te.setRelayPosition(stack);
                            player.sendMessage(new TextComponentString("Linked relay positions!"));
                        }
                    }
                }
            }
